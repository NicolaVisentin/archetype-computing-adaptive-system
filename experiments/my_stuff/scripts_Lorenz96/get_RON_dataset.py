...
n_hid = 15 # dimension of the hidden state (number of oscillators) (default RON: 1000)
architecture_to_test = 'lorenz_RON_full_15hidden' # path of the folder containing trained scaler, model and predictor
dt = 0.17 # dt of the RON reservoir (default RON: 0.17)
rho = 0.99 # spectral radius (default RON: 0.99)
inp_scaling = 0.1 # input matrix scaling (default RON: 0.1)
m = int(1e5) # dataset dimension (number of datapoints and labels)
y_range = [-0.12, 0.12] # range of positions to sample. To have an idea about the ranges, take a look at test_RON_model.py
yd_range = [-0.13, 0.13] # range of velocities to sample. To have an idea about the ranges, take a look at test_RON_model.py
...
u = -15 + 15 * torch.rand((m, n_inp), device=device) # m samples u. Shape (m, n_inp)
